...
export const tools = [
  ...
  definition("list_tickets", "List up to 250 accessible tickets, newest first. Optionally filter by exact project name, status, or search text. Use get_project for all tickets in one project.", object({ project: text(300), status, q: text(300) }), true),
  definition("get_ticket", "Read a ticket with comments, attachments, activity and current project role. Read before editing or resolving; confirm test evidence before marking Resolved.", object({ ticketId: id }, ["ticketId"]), true),
  ...
  definition("update_ticket", "Update specified ticket fields, including assignment and dates. Requires owner/editor permission. Status follows Created → Open → Started → Peer Review → QA Started → Resolved, with QA Issue → Started and Resolved → Open loops. Never resolve untested work.", object({ ticketId: id, ...ticketFields, status }, ["ticketId"]), false, true),
